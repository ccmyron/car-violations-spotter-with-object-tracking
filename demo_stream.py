...
# load the mask
mask = cv2.imread('assets/mask_ismail.png')
...
while cap.isOpened():
    ...
    # resize, to only operate with the zone of interest
    work_frame = work_frame[SHIFT_Y:, 0:800]
    ...
    for r in results:
        for box in r.boxes:
            ...
            x1, y1, x2, y2 = int(x1), int(y1) + SHIFT_Y, int(x2), int(y2) + SHIFT_Y
